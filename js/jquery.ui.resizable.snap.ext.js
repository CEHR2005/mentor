...
(function ($) {
	...
	$.ui.plugin.add('resizable', 'snap', {
		start: function () {
			...
			inst.bw = 2*parseFloat($(".selected").css("borderWidth"));
			inst.lm = getLm($this);
			inst.tm = getTm($this);
			inst.coords = [];
			...
			$(typeof snap == 'string' ? snap : ':data(ui-resizable)').each(function () {
				...
				inst.coords.push({ 
					l: l, t: t, 
					r: l + $el.outerWidth()+2*parseFloat($el.css("borderWidth")),
					b: t + $el.outerHeight()+2*parseFloat($el.css("borderWidth")) });
			});
		},
		resize: function () {
			var ls = [], ts = [], ws = [], hs = [],
				...
				r = l + inst.size.width + inst.bw + inst.ow, _r = r + st,
				b = t + inst.size.height + inst.bw + inst.oh, _b = b + st;
